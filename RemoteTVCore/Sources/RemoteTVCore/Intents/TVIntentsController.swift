...
public enum TVIntentError: Error, CustomLocalizedStringResourceConvertible {
...
@MainActor
public final class TVIntentsController {
    /// What ``togglePower()`` actually did, so the intent can word its dialog honestly.
    public enum PowerOutcome: Equatable {
    ...
    public static let interKeyDelay: Duration = .milliseconds(120)
    ...
    @MainActor public static var shared: TVIntentsController?
    ...
    @MainActor public static func resolve() -> TVIntentsController {
    ...
    @MainActor public static func makeStandalone() -> TVIntentsController {
        // Read the pairing token from the App Group (mirrored there by the app via
        // MirroringTokenStore). The extension can't see the app's Keychain, so reading the
        // Keychain here would miss the token and the TV would re-show its pairing popup.
    ...
    public func lastDevice() throws -> TVDevice {
        guard
    ...
    @discardableResult
    public func ensureConnected() async throws -> TVDevice {
    ...
    public func send(macro commands: [TVCommand]) async throws {
        try await ensureConnected()
    ...
    public func togglePower() async throws -> PowerOutcome {
        let device = try lastDevice()
        do {
            try await ensureConnected()
