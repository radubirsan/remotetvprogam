...
public actor FileRememberedTVsStore: RememberedTVsStore {
    private let url: URL
    private let encoder: JSONEncoder
    private let decoder: JSONDecoder

    public init(url: URL = URL.documentsDirectory.appending(path: "remembered-tvs.json")) {
        self.url = url
    ...
    public func all() async -> [RememberedTV] {
        readAll()
    }

    public func get(ip: String) async -> RememberedTV? {
        readAll().first { $0.ip == ip }
    }

    public func upsert(_ tv: RememberedTV) async throws {
        var records = readAll()
    ...
    public func delete(ip: String) async throws {
        var records = readAll()
