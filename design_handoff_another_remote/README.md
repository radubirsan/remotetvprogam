...
## Overview
"Another Remote" is an iPhone app that controls a smart TV. It has two main surfaces:
1. **Remote** — a skeuomorphic on-screen remote (Samsung-style button layout) with power, mic/voice, number pad, D-pad, transport, volume/channel rockers, and quick-launch app slots.
2. **TV Guide** — a scrollable list of channels showing what's on now and next, with a featured "live now" card, search, and category filters.
...
**Accent options** (user-selectable; default Ember):
| Name | hi | lo |
|---|---|---|
...
- `Another Remote.html` — full interactive prototype (open in a browser). Contains all four remote variations + the TV Guide screen. Use it to see exact spacing, colors, and behavior.
...
Open `Another Remote.html` first for the visual source of truth; the TV Guide is under the "App Screens" section.
