...
enum SharedStorage {
    /// Must match the App Group id added to BOTH the app and the control extension.
    static let appGroupID = "group.com.remotetv.RemoteTV"

    /// Keychain access group for sharing the pairing token with the extension. `nil` until
    /// Keychain Sharing is enabled (the group is normally `<TeamID>.com.remotetv.shared`);
    /// while `nil`, `KeychainTVTokenStore` behaves exactly as before. Mute-from-control needs
    /// this set, because it has to read the token to open the WebSocket; Wake-on-LAN power
    /// does not (it only needs the MAC, which travels inside the device JSON below).
    ...
    static let lastRemoteDeviceKey = "lastRemoteDeviceJSON"
    ...
    static var lastRemoteDeviceJSON: String? {
    ...
    static func setLastRemoteDeviceJSON(_ json: String) {
